...
class HederaAnonCredsRegistry(BaseAnonCredsResolver, BaseAnonCredsRegistrar):
    ...
    async def setup(self, context):
        ...
        client_provider: HederaClientProvider = get_client_provider(
                network,
                operator_id,
                operator_key_der
                )

        self._hedera_anoncreds_registry = SdkHederaAnonCredsRegistry(
                client_provider
                )
    ...
    async def register_schema(
        ...
        async with profile.session() as session:
            ...
            private_key_der = await get_private_key_der_from_did(
                    wallet,
                    issuer_did
                    )
