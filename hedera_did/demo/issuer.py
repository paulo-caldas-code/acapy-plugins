...
AGENT = Agent(
        ...
        wallet_webhook_url="http://issuer:3008/webhooks"
        )
CONNECTION_ID = None
DID = None
SCHEMA_ID = None
...
async def _interaction_loop():
    async def entrypoint():
        menu = Menu(
                ...
                entries=[
                    MenuEntry(key="1", description="Issue credential",      on_press=_issue_credential),
                    MenuEntry(key="2", description="Send Proof Request",    on_press=_send_proof_request),
                    MenuEntry(key="3", description="Send Message",          on_press=_send_message),
                    MenuEntry(key="4", description="Create New Invitation", on_press=_create_invitation),
                    MenuEntry(key="5", description="Revoke credential",     on_press=_revoke_credential)
                ]
            )
    ...
    async def _create_invitation() -> PressResult:
        ...
        if NGROK_ENDPOINT:
            invitation_url = invitation_url.replace("http://agent-issuer:3000", NGROK_ENDPOINT)
    ...
    async def _revoke_credential() -> PressResult:
        ...
        AGENT.revoke_credential(
                cred_ex_id=CRED_EX_ID,
                comment="Revoke demo credential"
                )
